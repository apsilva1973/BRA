...
type
  Tdmgcpcj_base_I = class(TDataModule)
    ...
    dts75: TADODataSet;
    adoConn57_ate_2016: TADOConnection;
    dts57_ate_2016: TADODataSet;
    adoCmd57_ate_2016: TADOCommand;
...
//alexandre 13/04
   ini := TIniFile.Create(ExtractFilePath(Application.exename) + 'config.ini');
   try
      adoConn57_ate_2016.ConnectionString := 'Provider=Microsoft.Jet.OLEDB.4.0;Password="";Data Source=' + ini.readstring('BaseDiaria_GCPJB057_ate_2016', 'databasename', '') + ';Persist Security Info=True';
   finally
      ini.free;
   end;
//alexandre 13/04
...
//alexandre 13/04
   try
      adoCmd57_ate_2016.CommandText := 'create index idxp10 on GCPJB057(CNRO_PROCS_BDSCO, CNRO_INSTN_PROCS)';
      adoCmd57_ate_2016.Execute;
   except
   end;
//alexandre 13/04
...
//alexandre 13/04
   try
      adoCmd57_ate_2016.CommandText := 'create index idxp20 on GCPJB057(CADVOG_INTRN_BDSCO_N)';
      adoCmd57_ate_2016.Execute;
   except
   end;
//alexandre 13/04
...
{   if dts57.RecordCount = 0 then
   ...
   if dts57.RecordCount = 0 then
    Begin
       dts57_ate_2016.Close;
       dts57_ate_2016.CommandText := 'SELECT CADVOG_INTRN_BDSCO_N ' +
                          'FROM GCPJB057 ' +
                          'WHERE CNRO_PROCS_BDSCO = ' + gcpj + ' AND CNRO_INSTN_PROCS=1 AND CSEQ_HIST_RESP IN(SELECT MAX(CSEQ_HIST_RESP) ' +
                          'FROM GCPJB057 WHERE CNRO_PROCS_BDSCO = ' + gcpj + ')';
       dts57_ate_2016.Open;

       if dts57_ate_2016.RecordCount = 0 then
        result := 0
       else
        result := dts57_ate_2016.FieldByName('CADVOG_INTRN_BDSCO_N').AsInteger;
    end
   else
    result := dts57.FieldByName('CADVOG_INTRN_BDSCO_N').AsInteger;
end;
