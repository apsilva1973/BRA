...
type
  ...
  public
    ...
    function ObtemEnvolvidosNoProcesso_2(gcpj: string; fgDrcAtivas: integer; buscarAutor: boolean;Nome_re : string):integer;
...
function Tdmgcpj_base_IX.ObtemEnvolvidosNoProcesso_2(gcpj: string; fgDrcAtivas: integer; buscarAutor: boolean;Nome_re : string):integer;
begin
//   if (fgdrcAtivas = 1) or (buscarAutor) then
//   begin
      dtsPesq.Close;
      ...
      dtsPesq.CommandText := 'select IENVDO_REFRD_PROCS, CENVDO_PROCS_AUTOR as CENVDO_PROCS_REU, CSEQ_PROCS_AUTOR as CSEQ_PROCS_REU ' +
                         'from GCPJB076 ' +
                         'where CNRO_PROCS_BDSCO = ' + gcpj + '  ' +
                         'AND IENVDO_REFRD_PROCS = ''' + Nome_re + '''  ' +
                         'order by CSEQ_PROCS_AUTOR';
      dtsPesq.Open;
      if dtsPesq.Eof then
         result := 0
