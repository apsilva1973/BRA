                     ...
                     begin
                        ...
                        begin
                           ...
                           begin
                              ...
                              if ret <> 1 then
                              begin
                                 ShowMessage('Valores digitados não conferem(1)');
                                 exit;
                              end
                              else
                              begin
                           ...
                           begin
                              ShowMessage('Valores digitados não conferem(2)');
                              exit;
                           end;
                        end;
                     end
...
(*      if (motivobaixa = 'ACORDO COM CUSTOS') or
   ...
   if (andamento = 'CARTA PRECATORIA') or (andamento = 'CARTA PRECATÓRIA') then
...
begin
   ...
   begin
      ...
      if index > 120 then
      begin
         ShowMessage('Não está na página correta(Campo)');
         exit;
      end;
   ...
   begin
      ShowMessage('Não está no formulário correto(2)');
      exit;
   end;
   ...
   if Not Assigned(inputElement) then
   begin
      ShowMessage('Não está no formulário correto(3)');
      exit;
   end;
...
begin
   ...
   begin
      ...
      begin
         ...
         if (FormatFloat('0.00', totaldigitadoTela) <> FormatFloat('0.00', TotalDigitadoDB)) then
         begin
            ShowMessage('Valores digitados não conferem(3)');
            exit;
         end;
      end;
      ...
      begin
         ShowMessage('Erro obtendo a dependência para digitar');
         exit;
      end;
...
//         planjbm.NotificaErro('Falta tipo ato');
      ...
      begin
//         planjbm.NotificaErro('Tipo de ato não encontrado no GCPJ');
         ShowMessage('Tipo de ato não encontrado no GCPJ');
         exit;
      end;
...
//      EsperaPaginaProcessar;
   ...
   if (FormatFloat('0.00', totaldigitadoTela) <> FormatFloat('0.00', TotalDigitadoDB)) then
   begin
      ShowMessage('Valores digitados não conferem(4): [' + FormatFloat('0.00', totaldigitadoTela) + '] comparando com [' + FormatFloat('0.00', TotalDigitadoDB) + ']');
      exit;
   end;
...
begin
   ...
   begin
      ...
      try
         ...
         begin
            ShowMessage('Nota ' + DBGrid2.DataSource.DataSet.FieldByName('numeronota').AsString + ' não localizada nas telas do GCPJ');
            exit;
         end;
      ...
      //se chegou aqui é pq não achou ainda
      valor := (wb.document as IHTMLDocument2).body.outerHTML;
      if Pos('página posterior', valor) = 0 then
      begin
         ShowMessage('Erro conectando ao GCPJ, não localizou a nota');
         exit;
      end;
...
begin
   ...
   Timer1.Enabled := true;
   try
      pJanelaFechar := UpperCase(ExtractFileName(Application.Exename)) + '|#32770|Mensagem da página da web';

      if ClicaBotaoGcpj(form, 'btoSalvar', UpperCase(ExtractFileName(Application.Exename)) + '|Static|Operação realizada com sucesso') < 0 then
      begin
         ShowMessage('Erro finalizando a nota');
         exit;
      end;
